#!/usr/bin/env bash
# ================================================================
#  displaylink — install pre-built kmod RPMs from kmods-zodium
...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◈  evdi Installer  ◈                   ║${NC}"
...
# ── Find the evdi.zip asset URL ───────────────────────────────
ASSET_URL="$(
  printf '%s' "${RELEASE_JSON}" \
  | python3 -c "
import json, sys
assets = json.load(sys.stdin).get('assets', [])
match = next((a['browser_download_url'] for a in assets if a['name'] == 'evdi.zip'), None)
if not match:
    raise SystemExit('evdi.zip not found in release assets')
print(match)
")" || fail "evdi.zip not found in release ${RELEASE_TAG} — kmods-zodium may still be building"

ok "Found asset: ${ASSET_URL}"

# ── Download & extract ────────────────────────────────────────
ZIP_PATH="${WORKDIR}/evdi.zip"
RPM_DIR="${WORKDIR}/rpms"
mkdir -p "${RPM_DIR}"

info "Downloading evdi.zip..."
...
[[ "${RPM_COUNT}" -gt 0 ]] || fail "No RPMs found inside evdi.zip"
...
say ""
say "${MAGENTA}${BOLD}╔══════════════════════════════════════════╗${NC}"
say "${MAGENTA}${BOLD}║   ◆  evdi Install Complete               ║${NC}"
say "${MAGENTA}${BOLD}╚══════════════════════════════════════════╝${NC}"
say ""