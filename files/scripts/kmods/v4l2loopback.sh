...
# ── Find the v4l2loopback.zip asset URL ──────────────────────
ASSET_URL="$(
  printf '%s' "${RELEASE_JSON}" \
  | python3 -c "
import json, sys
assets = json.load(sys.stdin).get('assets', [])
match = next((a['browser_download_url'] for a in assets if a['name'] == 'v4l2loopback.zip'), None)
if not match:
    raise SystemExit('v4l2loopback.zip not found in release assets')
print(match)
")" || fail "v4l2loopback.zip not found in release ${RELEASE_TAG} — kmods-zodium may still be building"

ok "Found asset: ${ASSET_URL}"

# ── Download & extract ────────────────────────────────────────
ZIP_PATH="${WORKDIR}/v4l2loopback.zip"
RPM_DIR="${WORKDIR}/rpms"
mkdir -p "${RPM_DIR}"

info "Downloading v4l2loopback.zip..."
...
[[ "${RPM_COUNT}" -gt 0 ]] || fail "No RPMs found inside v4l2loopback.zip"
